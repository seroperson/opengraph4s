 ...
 *
 * @param browser function which returns a [[Browser]] object which will be used
 *                to parse the given webpage. Can be either [[JsoupBrowser]]
 *                (without js support) or [[HtmlUnitBrowser]] (with js support).
 *                Take in mind that non-full webpage can be passed to [[parse()]],
 *                so using [[HtmlUnitBrowser]] can lead to errors during the
 *                parsing.
 * */
...
object ScalaScrapperParser {
  ...
  /**
   * Creates an object which uses [[JsoupBrowser]] or [[HtmlUnitBrowser]] browser
   * to parse the webpage.
   * */
  def apply[F[_] : MonadThrow](t: ScalaScrapperBrowserType): ScalaScrapperParser[F] =
    new ScalaScrapperParser(() => t match {
