 ...
 *
 * @see [[ScalaScrapperParser]] implementation using `scala-scrapper` library.
 *
 * @tparam C represents the type which this parser is able to parse.
...
trait Parser[C, F[_]] {
   ...
   *
   * @param data data to parse. It is not guaranteed that it represents a fully
   *             fetched webpage.
   *
   * @return parsed data.
   * */
