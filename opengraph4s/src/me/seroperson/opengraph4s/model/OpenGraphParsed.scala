...
/** Class represents parsed `head` element of a page.
  ...
  *   contains all the meta tags parsed from a page. Key is a value of
  *   `property` or `name` attribute, value is a value of `content` attribute.
  * @param titleTagFallback
  *   text of `title` tag if any.
  */
case class OpenGraphParsed(
  ...
  /** Holds a `og:title` or `title` value from meta tags. Or value from `title`
    * tag otherwise.
    */
  lazy val title = metaProperties
    .get("og:title")
  ...
  /** Holds a `og:description` or `description` value from meta tag.
    */
  lazy val description = metaProperties
    .get("og:description")
